...
``native_execution_enabled = true`` to take effect. On the Java path the optimization is a
no-op because the Java scan and Exchange operators do not perform inline widening.

The corresponding configuration property is :ref:`admin/properties:\`\`optimizer.push-down-widen-cast-enabled\`\``.
