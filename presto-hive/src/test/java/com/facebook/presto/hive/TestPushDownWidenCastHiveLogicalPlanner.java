        ...
        extends AbstractTestQueryFramework
{
    ...
    /**
     * Verifies the two post-fragmentation invariants the per-fragment rule is structured to
     * give us when a widening cast sits above an Exchange:
     *
     * <ol>
     *   <li><b>Wire format stays narrow.</b> The producing fragment's {@code PartitioningScheme.outputLayout}
     *       must keep the narrow type. No fragment's outputLayout is widened by this rewrite.</li>
     *   <li><b>The cast lands on the consumer's RemoteSource.</b> The consumer fragment's
     *       {@link com.facebook.presto.sql.planner.plan.RemoteSourceNode} declares the WIDE
     *       variable in its outputs (positionally where the narrow variable used to be), and
     *       the explicit {@code CAST(narrow AS wide)} above it is gone. The Exchange operator
     *       on the consumer side is responsible for emitting wide values from narrow-typed
     *       bytes received from the upstream task.</li>
     * </ol>
     *
     * <p>The pre-fragmentation registration of {@link
     * com.facebook.presto.sql.planner.optimizations.PushDownWidenCast} would have substituted
     * narrowVar with wideVar throughout the whole logical plan, including upstream of the
     * ExchangeNode, which would widen the wire format. Per-fragment pushdown bounds the
     * substitution to a fragment — so the producer fragment's outputLayout stays narrow while
     * the consumer fragment can still rewrite the cast onto its RemoteSource.
     */
    @Test
    public void testCastAboveExchangePushedIntoRemoteSourceConsumerSide()
    {
        // Aggregation forces a remote exchange between the partial- and final-aggregate
        // fragments. The widening cast sits above the aggregate. With per-fragment pushdown
        // the producer fragment's outputLayout stays INTEGER (the aggregate's narrow output)
        // and the cast lands on the consumer fragment's RemoteSourceNode (output widened to
        // BIGINT, wire format unchanged).
        ...
        verifyProducerOutputLayoutStaysNarrow(subPlan);
        verifyRemoteSourceConsumerSideWidened(subPlan);
    }

    /**
     * Walks every non-root fragment and asserts none of its {@code outputLayout} variables
     * is BIGINT — for this query the partial aggregate's output of {@code MIN(shippriority)}
     * is INTEGER, and the per-fragment rule must not widen it on the wire.
     */
    private static void verifyProducerOutputLayoutStaysNarrow(
            com.facebook.presto.sql.planner.SubPlan subPlan)
    {
        for (com.facebook.presto.sql.planner.SubPlan child : subPlan.getChildren()) {
            for (com.facebook.presto.spi.relation.VariableReferenceExpression output :
                    child.getFragment().getPartitioningScheme().getOutputLayout()) {
                assertFalse(
                        output.getType().equals(BIGINT),
                        "Producer fragment " + child.getFragment().getId()
                                + " outputLayout column " + output.getName() + " was widened to "
                                + output.getType()
                                + " — wire-format widening is the regression this rule prevents.");
            }
            verifyProducerOutputLayoutStaysNarrow(child);
        }
    }

    /**
     * Walks every fragment that has a RemoteSourceNode and asserts at least one such fragment
     * has a BIGINT-typed RemoteSource output — i.e. the consumer-side rewrite did fire and
     * the wide variable now arrives directly from the Exchange operator.
     */
    private static void verifyRemoteSourceConsumerSideWidened(
            com.facebook.presto.sql.planner.SubPlan subPlan)
    {
        boolean anyWidened = false;
        for (com.facebook.presto.sql.planner.plan.RemoteSourceNode remote :
                collectRemoteSources(subPlan)) {
            for (com.facebook.presto.spi.relation.VariableReferenceExpression output :
                    remote.getOutputVariables()) {
                if (output.getType().equals(BIGINT)) {
                    anyWidened = true;
                    break;
                }
            }
        ...
        assertTrue(
                anyWidened,
                "Expected at least one RemoteSourceNode to declare a BIGINT output after the "
                        + "per-fragment PushDownWidenCast rewrite, but no consumer-side widening "
                        + "was found. The cast above the Exchange was supposed to land on the "
                        + "RemoteSourceNode's declared outputs.");
    }

    private static java.util.List<com.facebook.presto.sql.planner.plan.RemoteSourceNode>
            collectRemoteSources(com.facebook.presto.sql.planner.SubPlan subPlan)
    {
        com.google.common.collect.ImmutableList.Builder<com.facebook.presto.sql.planner.plan.RemoteSourceNode>
                out = com.google.common.collect.ImmutableList.builder();
        out.addAll(subPlan.getFragment().getRemoteSourceNodes());
        for (com.facebook.presto.sql.planner.SubPlan child : subPlan.getChildren()) {
            out.addAll(collectRemoteSources(child));
        }
        return out.build();
    }
}
