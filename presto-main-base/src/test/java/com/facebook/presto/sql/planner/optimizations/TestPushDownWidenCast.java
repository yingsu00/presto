        ...
        extends BasePlanTest
{
    ...
    // -----------------------------------------------------------------------
    // Project -> RemoteSourceNode  (non-leaf fragment source)
    // -----------------------------------------------------------------------

    @Test
    public void testRemoteSourceWidenedDirectly()
    {
        // Project(CAST(narrow AS wide)) -> RemoteSourceNode(narrow)
        // Expected: Project(wide := wide) -> RemoteSourceNode(wide). The RemoteSource is
        // rewritten to declare the wide variable in its outputs; the producer fragment that
        // feeds it stays narrow on the wire (this rule only touches the consumer side).
        VariableReferenceExpression narrowVar = builder.variable("shippriority", INTEGER);
        VariableReferenceExpression wideVar = builder.variable("expr", BIGINT);

        RemoteSourceNode remote = builder.remoteSource(
                new com.facebook.presto.spi.plan.PlanNodeId("remote"),
                ImmutableList.of(new PlanFragmentId(1)),
                ImmutableList.of(narrowVar));

        PlanNode result = runOptimizer(
                builder.project(Assignments.of(wideVar, castExpr(narrowVar, BIGINT)), remote),
                sessionWithOptimizerEnabled());

        assertTrue(result instanceof ProjectNode);
        ProjectNode rp = (ProjectNode) result;
        // Cast collapsed to identity (wide := wide); InlineProjections would clean it up.
        assertEquals(rp.getAssignments().get(wideVar), wideVar);

        assertTrue(rp.getSource() instanceof RemoteSourceNode);
        RemoteSourceNode rewritten = (RemoteSourceNode) rp.getSource();
        // Wide variable replaces narrow positionally; everything else preserved.
        assertEquals(rewritten.getOutputVariables(), ImmutableList.of(wideVar));
        assertEquals(rewritten.getSourceFragmentIds(), remote.getSourceFragmentIds());
        assertEquals(rewritten.getExchangeType(), remote.getExchangeType());
        assertEquals(rewritten.getEncoding(), remote.getEncoding());
        assertEquals(rewritten.isEnsureSourceOrdering(), remote.isEnsureSourceOrdering());
        assertEquals(rewritten.getOrderingScheme(), remote.getOrderingScheme());
    }

    @Test
    public void testRemoteSourceWithIntermediateProjectWidened()
    {
        // Project(CAST(narrow AS wide)) -> Project(narrow := narrow) -> RemoteSource(narrow)
        // Expected: descent walks the intermediate identity Project and rewrites the
        // RemoteSource to output wide. The intermediate Project's identity assignment is
        // rewritten to wide := wide.
        VariableReferenceExpression narrowVar = builder.variable("shippriority", INTEGER);
        VariableReferenceExpression wideVar = builder.variable("expr", BIGINT);
        ...
        RemoteSourceNode remote = builder.remoteSource(
                ...
                ImmutableList.of(new PlanFragmentId(1)),
                ImmutableList.of(narrowVar));
        ProjectNode mid = builder.project(Assignments.of(narrowVar, narrowVar), remote);

        PlanNode result = runOptimizer(
                builder.project(Assignments.of(wideVar, castExpr(narrowVar, BIGINT)), mid),
                sessionWithOptimizerEnabled());

        ProjectNode top = (ProjectNode) result;
        assertEquals(top.getAssignments().get(wideVar), wideVar);

        ProjectNode rewrittenMid = (ProjectNode) top.getSource();
        // intermediate Project's narrow identity got rewritten to wide := wide.
        assertTrue(rewrittenMid.getAssignments().getMap().containsKey(wideVar));
        assertEquals(rewrittenMid.getAssignments().get(wideVar), wideVar);

        RemoteSourceNode rewrittenRemote = (RemoteSourceNode) rewrittenMid.getSource();
        assertEquals(rewrittenRemote.getOutputVariables(), ImmutableList.of(wideVar));
    }

    @Test
    public void testRemoteSourceOrderingKeyBailsOut()
    {
        // narrow var is in the merging RemoteSource's orderingScheme — changing its type
        // would change the merge collation, so the rule must bail.
        VariableReferenceExpression narrowVar = builder.variable("shippriority", INTEGER);
        VariableReferenceExpression wideVar = builder.variable("expr", BIGINT);
        ...
        RemoteSourceNode remote = new RemoteSourceNode(
                Optional.empty(),
                new com.facebook.presto.spi.plan.PlanNodeId("remote"),
                ImmutableList.of(new PlanFragmentId(1)),
                ImmutableList.of(narrowVar),
                ...
                com.facebook.presto.spi.plan.ExchangeEncoding.COLUMNAR);

        ProjectNode plan = builder.project(Assignments.of(wideVar, castExpr(narrowVar, BIGINT)), remote);
        PlanNode result = runOptimizer(plan, sessionWithOptimizerEnabled());

        // Nothing changed — the RemoteSource is the same instance, cast is still in the Project.
        ProjectNode top = (ProjectNode) result;
        assertSame(top.getSource(), remote);
        // Top assignment is still a CAST, not an identity.
        assertTrue(top.getAssignments().get(wideVar) instanceof CallExpression);
    }

    // -----------------------------------------------------------------------
    // Project -> LocalExchange -> RemoteSourceNode  (post-fragmentation chain)
    // -----------------------------------------------------------------------

    @Test
    public void testLocalExchangePushesThroughToRemoteSource()
    {
        // Project(CAST(narrow AS wide)) -> LocalExchange(GATHER, SINGLE) -> RemoteSource(narrow)
        // The planner inserts this LocalExchange between Project and RemoteSource in real
        // post-fragmentation plans. The optimizer must walk through it (it just routes data
        // between local pipelines, type-preserving) and rewrite the RemoteSource. The
        // LocalExchange's outputLayout / inputs[0] also swap narrow→wide at the same position.
        VariableReferenceExpression narrowVar = builder.variable("shippriority", INTEGER);
        VariableReferenceExpression wideVar = builder.variable("expr", BIGINT);
        ...
        ExchangeNode localExchange = builder.exchange(eb -> eb
                .type(ExchangeNode.Type.GATHER)
                .scope(ExchangeNode.Scope.LOCAL)
                .singleDistributionPartitioningScheme(narrowVar)
                .addSource(remote)
                .addInputsSet(narrowVar));

        PlanNode result = runOptimizer(
                builder.project(Assignments.of(wideVar, castExpr(narrowVar, BIGINT)), localExchange),
                sessionWithOptimizerEnabled());

        // Top Project's cast collapsed to identity wide := wide.
        ProjectNode top = (ProjectNode) result;
        assertEquals(top.getAssignments().get(wideVar), wideVar);

        // LocalExchange's outputLayout + inputs[0] both swapped to wideVar at position 0.
        assertTrue(top.getSource() instanceof ExchangeNode);
        ExchangeNode rewrittenExchange = (ExchangeNode) top.getSource();
        assertEquals(rewrittenExchange.getPartitioningScheme().getOutputLayout(), ImmutableList.of(wideVar));
        assertEquals(rewrittenExchange.getInputs(), ImmutableList.of(ImmutableList.of(wideVar)));
        assertEquals(rewrittenExchange.getScope(), ExchangeNode.Scope.LOCAL);
        assertEquals(rewrittenExchange.getType(), localExchange.getType());

        // RemoteSource below the LocalExchange has wideVar in outputs.
        assertTrue(rewrittenExchange.getSources().get(0) instanceof RemoteSourceNode);
        RemoteSourceNode rewrittenRemote = (RemoteSourceNode) rewrittenExchange.getSources().get(0);
        assertEquals(rewrittenRemote.getOutputVariables(), ImmutableList.of(wideVar));
    }

    @Test
    public void testLocalExchangePartitioningKeyBailsOut()
    {
        // narrow var is one of the LocalExchange's partitioning columns — its type drives
        // hash routing, so the rule must bail.
        VariableReferenceExpression narrowVar = builder.variable("shippriority", INTEGER);
        VariableReferenceExpression wideVar = builder.variable("expr", BIGINT);
        ...
        // LocalExchange hash-distributed on narrowVar.
        ExchangeNode localExchange = builder.exchange(eb -> eb
        ...
        ProjectNode plan = builder.project(Assignments.of(wideVar, castExpr(narrowVar, BIGINT)), localExchange);
        PlanNode result = runOptimizer(plan, sessionWithOptimizerEnabled());

        // Cast unchanged at top; LocalExchange unchanged (same instance).
        ProjectNode top = (ProjectNode) result;
        assertSame(top.getSource(), localExchange);
        assertTrue(top.getAssignments().get(wideVar) instanceof CallExpression);
    }
}
