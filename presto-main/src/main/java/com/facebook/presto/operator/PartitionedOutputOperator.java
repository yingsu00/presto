        ...
        implements Operator
{
    ...
    private Logger log = Logger.get(PartitionedOutputOperator.class);;
    ...
    private static class PartitionData
    {
        ...
        private int[] positions = new int[100];   // 0.. positionCount-1
        private int[] rowSizes = new int[100];
        ...
        private Logger log = Logger.get(PartitionData.class);
        ...
        private void createBuffers(Page page)
        {
            if (blockEncodingBuffers == null) {
                ...
                for (int i = 0; i < channelCount; i++) {
                    blockEncodingBuffers[i] = createBlockEncodingBuffers(page.getBlock(i));
                }
            }
        }
        ...
        private void setupPositionsInBuffers()
        {
            for (int i = 0; i < blockEncodingBuffers.length; i++) {
                blockEncodingBuffers[i].setupPositions(positions, positionCount);
            }
        }
        ...
        private int calculateNumberRowsFitInBuffers( int[] positionSizesInBytes, int startPosition)
        {
            // TODO: validate rows and rowSizes length >= numberOfRows > firstRowIndex
            int remainingBytes = capacity - getBuffersSizeInBytes();
        ...
        public void ensurePositionsCapacity(Page page)
        {
            if (positions == null || positions.length < page.getPositionCount()) {
                positions = Arrays.copyOf(positions, (int) 1.2 * page.getPositionCount());
            }
        }
        ...
        private Slice serialize()
        {
            ...
            Slice slice = Slices.wrappedBuffer(new byte[getBuffersSizeInBytes()]);
    ...
    private static class PagePartitioner
    {
        ...
        private void populatePositionsForEachPartition(Page page)
        {
            for (int i = 0; i < partitionData.length; i++) {
                ...
                target.ensurePositionsCapacity(page);
            }
