...
public class BenchmarkPartitionedOutputOperatorAppendToBuffers
{
    ...
    public void addLongValuesToByteArrayBuffer(BenchmarkData data)
    {
        for (int i = 0; i < RUNS; i++) {
            int[][] longValueBufferIndexes = new int[data.PARTITION_COUNT][data.CHANNEL_COUNT];
            for (int[] longValueBufferIndex : longValueBufferIndexes) {
                Arrays.fill(longValueBufferIndex, ARRAY_BYTE_BASE_OFFSET);
            }

            for (int pageIndex = 0; pageIndex < data.getPageCount(); pageIndex++) {
                for (int channelIndex = 0; channelIndex < data.CHANNEL_COUNT; channelIndex++) {
...
//        for (int i = 0; i < RUNS; i++) {
//            int[][] longValueBufferIndexes = new int[data.PARTITION_COUNT][data.CHANNEL_COUNT];
//            Arrays.fill(longValueBufferIndexes, ARRAY_BYTE_BASE_OFFSET);
//
//            int[][] shortValueBufferIndexes = new int[data.PARTITION_COUNT][data.CHANNEL_COUNT];
//            Arrays.fill(shortValueBufferIndexes, ARRAY_BYTE_BASE_OFFSET);
//
//            for (int pageIndex = 0; pageIndex < data.getPageCount(); pageIndex++) {
//                for (int channelIndex = 0; channelIndex < data.CHANNEL_COUNT; channelIndex++) {
//                    long[] longValues = data.getLongValues(pageIndex, channelIndex);
...
//                for (int channelIndex = 0; channelIndex < data.CHANNEL_COUNT; channelIndex++) {
//                Block bigintBlock = page.getBlock(0);
//                Block smallintBlock = page.getBlock(1);
//                for (int partition = 0; partition < data.PARTITION_COUNT; partition++) {
//                    int[] positions = data.getPositions(partition);
//                    int positionCount = data.getPostistionCount(partition);
//
...
//    }
    ...
    public static class BenchmarkData
    {
        ...
        public Page getShortDataPage(int pageIndex)
        {
            return shortDataPages[pageIndex];
        }
        ...
        private void createBigintPages()
        {
            ...
            for (int pageIndex = 0; pageIndex < PAGE_COUNT; pageIndex++) {
                Block[] blocks = new Block[CHANNEL_COUNT];
        ...
        private void createMixedTypesPages()
        {
            ...
            types = ImmutableList.of(BIGINT, SMALLINT);
            for (int pageIndex = 0; pageIndex < PAGE_COUNT; pageIndex++) {
                Block[] blocks = new Block[CHANNEL_COUNT * 2];
        ...
        {
            longValues = new long[PAGE_COUNT][CHANNEL_COUNT][POSITIONS_PER_PAGE];
            for (int pageIndex = 0; pageIndex < PAGE_COUNT; pageIndex++) {
                for (int channelIndex = 0; channelIndex < CHANNEL_COUNT; channelIndex++) {
                    for (int position = 0; position < POSITIONS_PER_PAGE; position++) {
        ...
        {
            shortValues = new short[PAGE_COUNT][CHANNEL_COUNT][POSITIONS_PER_PAGE];
            for (int pageIndex = 0; pageIndex < PAGE_COUNT; pageIndex++) {
                for (int channelIndex = 0; channelIndex < CHANNEL_COUNT; channelIndex++) {
                    for (int position = 0; position < POSITIONS_PER_PAGE; position++) {
        ...
        {
            longValuesBuffers = new byte[PARTITION_COUNT][CHANNEL_COUNT][PAGE_COUNT * POSITIONS_PER_PAGE * ARRAY_LONG_INDEX_SCALE];
        }

        private void createShortByteArrayBuffers()
        {
            shortValuesBuffers = new byte[PARTITION_COUNT][CHANNEL_COUNT][PAGE_COUNT * POSITIONS_PER_PAGE * ARRAY_SHORT_INDEX_SCALE];
        }
