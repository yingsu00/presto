...
public class ByteArrayUtils
{
    ...
    public static int putShortValuesToBuffer(short[] values, int[] positions, int positionCount, int offsetBase, byte[] target, int targetIndex)
    {
        //if (sizeOf(target) < targetIndex + positionCount * ARRAY_LONG_INDEX_SCALE || targetIndex < 0) {
        if (target.length < targetIndex - ARRAY_BYTE_BASE_OFFSET + positionCount * ARRAY_SHORT_BASE_OFFSET || targetIndex < 0) {
            throw new IndexOutOfBoundsException();
        }
